...
- **Deliverables:** the three-attempt choice of §3.1 — `port.txt`, then a SHA-256-of-SID derivation, then a bounded walk; each walk step classifying the occupant through the `/health` identity; the bound port written to `port.txt` and carried into the hook URL; `DashboardSettings.DefaultPort`'s remark corrected, since its stated reason no longer holds. **Superseded in part by T1.28** — there is no hook URL now, and the bound port is carried into `listening.txt` instead (issue #29). Left as written because it records what T1.21 delivered.
