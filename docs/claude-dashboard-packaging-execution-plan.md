...
**First iteration — the operator's machine, 2026-09-02 02:37–02:40, run by the director (D112).** Packaged from `10cc1e3` with `build\package.ps1 -Version 0.1.0` (11 s). `Setup.exe --silent`: exit 0 in 3 s, no elevation prompt, no SmartScreen (a locally built file carries no mark of the web). Observed, in the gate's numbering: **2** no prompt of any kind; **3** launched from the shortcut's target, tray and window up, the log's first line `Claude Dashboard 0.1.0+10cc1e3…`; **4** `current\` (417 files), `Update.exe`, `packages\` and the launcher stub under `%LocalAppData%\dsopko.ClaudeDashboard\`, nothing elsewhere; **5** the existing data root reused — settings, database, logs, `post-status.cmd` all as they were; **6** `Claude Dashboard.lnk` in the Start Menu root, the Apps entry titled *Claude Dashboard*, version 0.1.0, publisher *David Sopko*, **no desktop shortcut** (D113); **7** not applicable here — Claude Code is installed, the handler was complete, and **Claude Code's `settings.json` is byte-identical before and after** (md5 `467A226C…`, mtime unchanged), which is T1.32's no-write path observed on a real file for the first time. Ingress on 52888, announced, `/health` ok, and **hook events from the machine's live Claude Code sessions were reaching the new install within a minute** — the database's write time advanced with no synthetic event sent. The legacy `%LocalAppData%\ClaudeDashboardApp\` and `.staging` (180 MB each) were deleted after the new process was confirmed running from the Velopack path.

**Not observed here, still the gate's:** **1** SmartScreen on a downloaded copy; **8** a relaunch from the Start Menu by hand; **9** uninstall from Settings → Apps; **10** the portable zip on a machine that is not this one (measured redirected in PKG.3); the standard-user run, which needs the VM. Two facts for whoever runs it: **a `--silent` install does not launch the app** — the app was started by hand from the shortcut's target, so whether the ordinary double-click Setup launches it is unobserved; and the Generic Host's *content root* is the launching shell's cwd, which is cosmetic — every path the app uses comes from the data folder or the executable's own directory.
